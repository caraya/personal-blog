...
The `$defs` object holds definitions that are common to more than one object in the schema. The interview schema defines a `person` object for both the `interviewer` and `interviewee` arrays.
