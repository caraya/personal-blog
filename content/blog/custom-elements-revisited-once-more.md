---
title: "Custom Elements revisited (once more)"
date: "2019-08-26"
---
...
- **Custom Properties** provides a way to create reusable styles that will work through the shadow DOM

## Reviewing Custom elements

The idea behind custom elements is to allow developers to create their valid HTML elements that encapsulate HTML content, CSS styles and Javascript behavior that is unique to that instance of the component.
...
[Slots and named slots](https://alligator.io/web-components/composing-slots-named-slots/) are the answer to this question. Using slots we're telling the browser: **_"Whatever content matches the name of this slot will replace the content of the slot in the template"_**.
...
This makes the element portable as we don't have to paste the template on every page where we'll use it. It also helps in keeping the styles for the component in one place, particularly since the styles will not be needed anywhere else.
...
`attributeChangedCallback` checks if the width and height attributes exist at all and, if they exist, checks if they are empty. If either of those conditions is true then we set a default value. If not it assumes that the values are correct and leaves them as is.
...
- [::part and ::theme, an ::explainer](https://meowni.ca/posts/part-theme-explainer/) — Monica Dinculescu
