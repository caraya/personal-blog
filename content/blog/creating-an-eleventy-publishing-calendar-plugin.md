---
title: Creating An Eleventy Publishing Calendar Plugin
date: 2026-01-12
tags:
  - Eleventy
  - Plugin
...
### 5. Post Grouping and Serialization

Posts are grouped by their publication date and serialized for client-side use.
...
This creates an object where keys are ISO date strings and values are arrays of simplified post objects (title, URL, filename, content type) for that date. This data is later injected into the client-side script.
...
This creates a navigation bar with Previous/Next links and the current month/year title. Note the unique `id` assigned to the container and the specific classes for navigation links, which are used by the client-side script.
...
while (Temporal.PlainDate.compare(date, endOfWeek) <= 0) {
  // ... (logic to determine classes and background color)
...
To enable dynamic navigation without page reloads, the plugin injects a client-side script.
...
(function() {
    function initCalendar() {
        // ... (calendar rendering logic using Temporal)
    }
    ...
    if (typeof Temporal !== 'undefined') {
        ...
        if (!document.getElementById('temporal-polyfill')) {
            ...
            script.src = 'https://cdn.jsdelivr.net/npm/@js-temporal/polyfill/dist/index.umd.js';
...
1. **Checks for Temporal Support**: It verifies if `Temporal` is available natively or via an existing polyfill.
2. **Dynamic Polyfill Loading**: If `Temporal` is missing, it dynamically loads the polyfill from a CDN.
3. **Event Handling**: It listens for clicks on "Previous" and "Next" links, updates the state, and re-renders the calendar grid on the client side using the embedded `postsData`.
...
This provides basic styling for the calendar layout, colors, and typography.
...
- **Overall Trade-off**: This approach favors simplicity over optimization, suitable for most Eleventy projects. Performance is typically acceptable unless dealing with exceptionally large content volumes.
